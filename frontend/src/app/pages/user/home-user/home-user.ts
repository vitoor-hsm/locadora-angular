...
export class HomeUserComponent implements OnInit {
  ...
  loadMovies(sortByNew: boolean = this.isSortedByNew, page: number = 0) {
    // 1. Retrieves the ID of the logged-in user
    ...
    // Depending on the filter, calls the appropriate method in AuthService
  ...
  toggleFavorite(movie: any) {
    ...
    this.authService.toggleFavorite(movie.id).subscribe({
      next: () => {
        // Inverts the current value (if it's null/undefined, it becomes true)
        const novoEstado = !movie.favorite && !movie.isFavorite;

        // Update ALL name variations to avoid errors
        movie.favorite = novoEstado;
        movie.isFavorite = novoEstado;

        console.log("Estado atualizado para:", novoEstado);

        // It forces Angular to redraw the component on the fly
        this.cdr.detectChanges();
      },
  ...
  clearFilter() {
    // Inform the Service that the favorites filter is now FALSE 
